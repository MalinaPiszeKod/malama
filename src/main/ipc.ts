...
import type { ChatSession, LlamaServerSettings } from '../shared/types';
...
export function registerIpc(context: IpcContext): void {
  ...
  ipcMain.handle(IPC_CHANNELS.saveSettings, async (_event, settings) => context.settings.save(objectPayload<Partial<LlamaServerSettings>>(settings, 'settings')));
  ipcMain.handle(IPC_CHANNELS.resetSettings, async () => context.settings.reset());
  ipcMain.handle(IPC_CHANNELS.refreshCatalog, async () => context.models.refreshCatalog(await context.settings.loadOverrides()));
  ...
  ipcMain.handle(IPC_CHANNELS.launchModel, async (_event, payload: unknown) => {
    const data = objectPayload<{ modelId?: unknown }>(payload, 'launch payload');
    const modelId = data.modelId === undefined ? undefined : stringPayload(data.modelId, 'modelId', 1024);
    return context.launcher.start(await context.models.findModel(modelId), await context.settings.loadOverrides());
  });
  ...
  ipcMain.handle(IPC_CHANNELS.chatSend, async (_event, payload: unknown) => {
    ...
    return context.chat.sendMessage(sessionId, message, await context.settings.load(), context.launcher.snapshot.modelId);
  });
  ...
  ipcMain.handle(IPC_CHANNELS.hfSearch, async (_event, query: unknown) => context.huggingFace.search(stringPayload(query, 'query', 256)));
